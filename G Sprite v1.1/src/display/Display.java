	...
	public void paint(Graphics g) {
		...
		if (handler.getProjectAmount() > 0) {
			if (handler.getSpriteAmount() > 0) {
				...
				for (int i = 0; i < handler.getLineSize(); i++) {
					for (int j = 0; j < handler.getLineSize(); j++) {
						c = handler.getBoxColor(i, j);
						if (!showRender || !(c.equals(Color.decode("0xFF0096")))) {
							g.setColor(c);
							g.fillRect(handler.boxSize * j + handler.xAxis, handler.boxSize * i + handler.yAxis, handler.boxSize, handler.boxSize);
						}

						if (showGrid) {
							g.setColor(Color.BLACK);
							g.drawRect(handler.boxSize * j + handler.xAxis, handler.boxSize * i + handler.yAxis, handler.boxSize, handler.boxSize);
						}
		...
		// Draw Buttons
				for (int i = 0; i < handler.getButtonSize(); i++) {
					...
					if (handler.but.visible) {
						g.drawImage(handler.getButtonImage(handler.but.sprite), handler.but.x, handler.but.y, handler.but.wid, handler.but.hei, this);
					}
		...
		if (handler.getProjectAmount() > 0) {
			for (int i = 0; i < handler.getSpriteAmount(); i++) {
				...
				g.drawImage(handler.getSpriteImage(i + handler.getScroll()), (int) (handler.but.x * 1.002), (int) (handler.but.y * 1.05), box, box, this);
