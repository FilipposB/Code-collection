...
public class Handler {
	...
	private void floodFill(int x, int y, Color targetColor, Color replacementColor, boolean mark[][]) {
		...
		if (x < proj.get(selProject).sprite.get(selSprite).lineSize - 1)
			floodFill(x + 1, y, targetColor, replacementColor, mark);
		if (y > 0)
			floodFill(x, y - 1, targetColor, replacementColor, mark);
		if (y < proj.get(selProject).sprite.get(selSprite).lineSize - 1)
			floodFill(x, y + 1, targetColor, replacementColor, mark);
	...
	public BufferedImage getSpriteImage(int spr) {
		...
		BufferedImage image = new BufferedImage(proj.get(selProject).sprite.get(spr).lineSize, proj.get(selProject).sprite.get(spr).lineSize, BufferedImage.TYPE_INT_ARGB);
		int[] pixels = ((DataBufferInt) image.getRaster().getDataBuffer()).getData();
		render.clear();
