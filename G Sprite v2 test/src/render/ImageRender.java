...
public class ImageRender {
	...
	public void renderSprite(Sprite sprit, ColorArchive color) {
		sprPixels = new int[sprit.lineSize * sprit.lineSize];
		int p;
		for (int i = 0; i < sprit.lineSize; i++) {
			for (int j = 0; j < sprit.lineSize; j++) {
				...
				sprPixels[j + i * sprit.lineSize] = (p << 24) | (RGBcolors[0] << 16) | (RGBcolors[1] << 8) | RGBcolors[2];
			}
		}
	}
	public void refreshSprite(int x, ColorArchive color, int col) {
		int p;
		p=255;
		RGBcolors[0] = color.color.get(col).getRed();
		RGBcolors[1] = color.color.get(col).getGreen();
		RGBcolors[2] = color.color.get(col).getBlue();
		sprPixels[x]= (p << 24) | (RGBcolors[0] << 16) | (RGBcolors[1] << 8) | RGBcolors[2];
